<section>
  @include('components.breadcrumb', ['active' => __('main.managements')])
    ...
    <div class="card-body">
      <table class="table table-bordered">
        ...
        @foreach($managements as $management)
          ...
          <tr>
            <td>{{ ($managements->currentPage()-1) * $managements ->perpage() + $loop->index +1 }}.</td>
            <td>{{ $management->firstname }}</td>
            <td>
              <div class="d-flex">
                @can('view', \App\Livewire\ManagementComponent::class)
                  <button wire:click="show({{ $management->id }})" class="btn btn-sm btn-light"><i class="fas fa-eye"></i></button>
                @endcan
                @can('update', \App\Livewire\ManagementComponent::class)
                  <button wire:click="edit({{ $management->id }})" class="btn btn-sm btn-light mx-2"><i class="fas fa-pencil-alt text-primary"></i></button>
                @endcan
                @can('destroy', \App\Livewire\ManagementComponent::class)
                  <button wire:click="delete({{ $management->id }})" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>
                @endcan
              </div>
            </td>
          </tr>
