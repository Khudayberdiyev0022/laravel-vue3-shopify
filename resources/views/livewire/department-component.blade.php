<section>
  @include('components.breadcrumb', ['active' => __('main.departments')])
  @include('components.alert')
  <div class="card">
    <div class="card-header">
      <div class="d-flex justify-content-between">
        <h4>{{ __('main.lists') }}</h4>
        @can('create', \App\Livewire\DepartmentComponent::class)
          ...
          <tr>
            ...
            <td>
              @forelse($department->positions as $position)
                {{ $position->title }} ,
              @empty
                <span class="badge badge-danger p-2">{{ __('main.no_position') }}</span>
              @endforelse
            </td>
            <td>
              <div class="d-flex">
                @can('view', \App\Livewire\DepartmentComponent::class)
                  <button wire:click="show({{ $department->id }})" class="btn btn-sm btn-light"><i class="fas fa-eye"></i></button>
                @endcan
                @can('update', \App\Livewire\DepartmentComponent::class)
                  <button wire:click="edit({{ $department->id }})" class="btn btn-sm btn-light mx-2"><i class="fas fa-pencil-alt text-primary"></i></button>
                @endcan
                @can('destroy', \App\Livewire\DepartmentComponent::class)
                  <button wire:click="delete({{ $department->id }})" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>
                @endcan
              </div>
            </td>
          </tr>
