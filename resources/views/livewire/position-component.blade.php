<section>
  @include('components.breadcrumb', ['active' => __('main.positions')])
  @include('components.alert')
  <div class="card">
    <div class="card-header">
      <div class="d-flex justify-content-between">
        <h4>{{ __('main.lists') }}</h4>
        @can('create', \App\Livewire\PositionComponent::class)
      ...
      <table class="table table-bordered">
        ...
        @foreach($positions as $position)
          <tr>
            ...
            <td>
              <div class="d-flex">
                @can('view', \App\Livewire\PositionComponent::class)
                  <button wire:click="show({{ $position->id }})" class="btn btn-sm btn-light"><i class="fas fa-eye"></i></button>
                @endcan
                @can('update', \App\Livewire\PositionComponent::class)
                  <button wire:click="edit({{ $position->id }})" class="btn btn-sm btn-light mx-2"><i class="fas fa-pencil-alt text-primary"></i></button>
                @endcan
                @can('destroy', \App\Livewire\PositionComponent::class)
                  <button wire:click="delete({{ $position->id }})" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>
                @endcan
              </div>
            </td>
          </tr>
    ...
    <x-slot name="body">
      <form wire:submit="store">
        <div class="form-group">
          <label class="required" for="department_id">{{ __('main.department') }}</label>
          <select wire:model="department_id" class="form-control @error('department_id') is-invalid @enderror" id="department_id">
