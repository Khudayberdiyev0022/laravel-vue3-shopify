<section>
  @include('components.breadcrumb', ['active' => __('main.chairs')])
      ...
      <table class="table table-bordered">
        ...
        @foreach($chairs as $chair)
          <tr>
            ...
            <td>
              <div class="d-flex">
                @can('view', \App\Livewire\ChairComponent::class)
                  <button wire:click="show({{ $chair->id }})" class="btn btn-sm btn-light"><i class="fas fa-eye"></i></button>
                @endcan
                @can('update', \App\Livewire\ChairComponent::class)
                  <button wire:click="edit({{ $chair->id }})" class="btn btn-sm btn-light mx-2"><i class="fas fa-pencil-alt text-primary"></i></button>
                @endcan
                @can('destroy', \App\Livewire\ChairComponent::class)
                  <button wire:click="delete({{ $chair->id }})" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>
                @endcan
              </div>
            </td>
          </tr>
