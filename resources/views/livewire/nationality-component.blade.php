<section>
  @include('components.breadcrumb', ['active' => __('main.nationalities')])
  @include('components.alert')
  <div class="card">
    <div class="card-header">
      <div class="d-flex justify-content-between">
        <h4>{{ __('main.lists') }}</h4>
        @can('create', \App\Livewire\NationalityComponent::class)
    ...
    <div class="card-body">
      <table class="table table-bordered">
        ...
        @foreach($nationalities as $nationality)
          <tr>
            <td>{{ ($nationalities->currentPage()-1) * $nationalities ->perpage() + $loop->index +1 }}.</td>
            <td>{{ $nationality->title }}</td>
            <td>
              <div class="d-flex">
                @can('view', \App\Livewire\NationalityComponent::class)
                  <button wire:click="show({{ $nationality->id }})" class="btn btn-sm btn-light"><i class="fas fa-eye"></i></button>
                @endcan
                @can('update', \App\Livewire\NationalityComponent::class)
                  <button wire:click="edit({{ $nationality->id }})" class="btn btn-sm btn-light mx-2"><i class="fas fa-pencil-alt text-primary"></i></button>
                @endcan
                @can('destroy', \App\Livewire\NationalityComponent::class)
                  <button wire:click="delete({{ $nationality->id }})" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>
                @endcan
              </div>
            </td>
          </tr>
