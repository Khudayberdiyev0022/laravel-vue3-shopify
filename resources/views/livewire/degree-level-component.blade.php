<section>
  @include('components.breadcrumb', ['active' => __('main.degree_levels')])
  <div class="card">
    <div class="card-header">
      <div class="d-flex justify-content-between">
        <h4>{{ __('main.lists') }}</h4>
        @can('create', \App\Livewire\DegreeLevelComponent::class)
    ...
    <div class="card-body">
      <table class="table table-bordered">
        ...
        @foreach($degreelevels as $degreelevel)
          <tr>
            <td>{{ ($degreelevels->currentPage()-1) * $degreelevels ->perpage() + $loop->index +1 }}.</td>
            <td>{{ $degreelevel->title }}</td>
            <td>
              <div class="d-flex">
                @can('view', \App\Livewire\DegreeLevelComponent::class)
                  <button wire:click="show({{ $degreelevel->id }})" class="btn btn-sm btn-light"><i class="fas fa-eye"></i></button>
                @endcan
                @can('update', \App\Livewire\DegreeLevelComponent::class)
                  <button wire:click="edit({{ $degreelevel->id }})" class="btn btn-sm btn-light mx-2"><i class="fas fa-pencil-alt text-primary"></i></button>
                @endcan
                @can('destroy', \App\Livewire\DegreeLevelComponent::class)
                  <button wire:click="delete({{ $degreelevel->id }})" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>
                @endcan
              </div>
            </td>
          </tr>
  ...
  <x-modal id="create">
    ...
    <x-slot name="body">
      <form wire:submit="store">
        ...
        <div class="form-group">
          <button type="submit" class="btn btn-primary" style="float: right"><i class="fas fa-save mr-1"></i></button>
        </div>
      </form>
    </x-slot>
  </x-modal>
